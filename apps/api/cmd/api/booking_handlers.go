...
func (app *App) handleBookingsApprove(w http.ResponseWriter, r *http.Request) {
	...
	if !canApproveBooking(user.Role) { writeJSON(w, http.StatusForbidden, map[string]string{"message": "Manager role required to approve booking"}); return }
	booking, err := app.bookingByID(r.Context(), chi.URLParam(r, "id"))
	...
	if booking.BAID != nil {
		...
		if !allowed { writeJSON(w, http.StatusBadRequest, map[string]string{"message": fmt.Sprintf("Cannot approve booking because capacity exceeds 100%% on %s", blockingDay)}); return }
	}
...
func (app *App) handleBookingsReject(w http.ResponseWriter, r *http.Request) {
	user, err := app.currentUser(r)
	if err != nil { writeJSON(w, http.StatusUnauthorized, map[string]string{"message": "Authentication required."}); return }
	if !canApproveBooking(user.Role) { writeJSON(w, http.StatusForbidden, map[string]string{"message": "Manager role required to reject booking"}); return }
	var body map[string]string
	_ = decodeJSON(r, &body)
	reason := strings.TrimSpace(body["reject_reason"])
	if reason == "" { writeJSON(w, http.StatusBadRequest, map[string]string{"message": "reject_reason is required"}); return }
...
func (app *App) handleBookingsAssign(w http.ResponseWriter, r *http.Request) {
	user, err := app.currentUser(r)
	if err != nil { writeJSON(w, http.StatusUnauthorized, map[string]string{"message": "Authentication required."}); return }
	if !canAssignBooking(user.Role) { writeJSON(w, http.StatusForbidden, map[string]string{"message": "Manager role required to assign booking"}); return }
	var input bookingInput
	...
	if input.BAID == nil || strings.TrimSpace(*input.BAID) == "" { writeJSON(w, http.StatusBadRequest, map[string]string{"message": "ba_id is required"}); return }
	ba, err := app.loadBAProfile(r.Context(), strings.TrimSpace(*input.BAID))
	if err != nil || ba.Status != "ACTIVE" { writeJSON(w, http.StatusBadRequest, map[string]string{"message": "Only active BA can be assigned"}); return }
	booking, err := app.bookingByID(r.Context(), chi.URLParam(r, "id"))
	if err != nil { writeJSON(w, http.StatusNotFound, map[string]string{"message": "Booking not found"}); return }
	...
	if !allowed { writeJSON(w, http.StatusBadRequest, map[string]string{"message": fmt.Sprintf("Cannot assign BA because capacity exceeds 100%% on %s", blockingDay)}); return }
...
func (app *App) handleBookingsMyRequests(w http.ResponseWriter, r *http.Request) {
	user, err := app.currentUser(r)
	if err != nil { writeJSON(w, http.StatusUnauthorized, map[string]string{"message": "Authentication required."}); return }
	if user.Role != "PM_PO" && !canApproveBooking(user.Role) { writeJSON(w, http.StatusForbidden, map[string]string{"message": "PM/PO or Manager role required"}); return }
...
func (app *App) handleBookingsMySchedule(w http.ResponseWriter, r *http.Request) {
	...
	ba, err := app.baProfileByUserID(r.Context(), user.ID)
	if err != nil {
		if canApproveBooking(user.Role) { writeJSON(w, http.StatusOK, []any{}); return }
		writeJSON(w, http.StatusForbidden, map[string]string{"message": "BA profile not found for current user"})
		return
	}
...
func (app *App) queryBookings(ctx context.Context, query string, args ...any) ([]Booking, error) {
	...
	for rows.Next() {
		...
		if project, err := app.loadProject(ctx, booking.ProjectID); err == nil { booking.Project = project }
		if requester, err := app.findUserByID(ctx, booking.RequesterID); err == nil { booking.Requester = requester }
		if booking.ManagerID != nil { if manager, err := app.findUserByID(ctx, *booking.ManagerID); err == nil { booking.Manager = manager } }
...
func (app *App) bookingByID(ctx context.Context, id string) (*Booking, error) {
	...
	if err != nil || len(items) == 0 { return nil, fmt.Errorf("booking not found") }
	return &items[0], nil
}
...
func (app *App) canReadBooking(ctx context.Context, user *User, booking *Booking) bool {
	if canApproveBooking(user.Role) || booking.RequesterID == user.ID { return true }
	if user.Role == "BA" && booking.BAID != nil {
		if ba, err := app.baProfileByUserID(ctx, user.ID); err == nil && ba.ID == *booking.BAID { return true }
	}
	return false
}
...
func (app *App) normalizeBookingInput(ctx context.Context, input bookingInput) (*bookingInputNormalized, map[string]any, error) {
	...
	if input.BAID != nil && strings.TrimSpace(*input.BAID) != "" {
		...
		if err != nil || ba.Status != "ACTIVE" { return nil, nil, fmt.Errorf("Only active BA can be booked") }
		baID = &trimmed
	}
	if input.StartDate == nil || input.EndDate == nil { return nil, nil, fmt.Errorf("start_date and end_date are required") }
	startDate, err := parseDateOnly(strings.TrimSpace(*input.StartDate)); if err != nil { return nil, nil, err }
	endDate, err := parseDateOnly(strings.TrimSpace(*input.EndDate)); if err != nil { return nil, nil, err }
	if endDate.Before(startDate) { return nil, nil, fmt.Errorf("end_date must be greater than or equal to start_date") }
	projectID := ""
	...
	if input.Title == nil || strings.TrimSpace(*input.Title) == "" { return nil, nil, fmt.Errorf("title is required") }
	if input.Description == nil || strings.TrimSpace(*input.Description) == "" { return nil, nil, fmt.Errorf("description is required") }
	if input.CapacityPercent == nil || !allowedCapacity(*input.CapacityPercent) { return nil, nil, fmt.Errorf("capacity_percent must be 25, 50, 75, or 100") }
	priority := "MEDIUM"
	if input.Priority != nil && strings.TrimSpace(*input.Priority) != "" { priority = strings.TrimSpace(*input.Priority) }
...
func allowedCapacity(value int) bool { return value == 25 || value == 50 || value == 75 || value == 100 }
...
func (app *App) insertBooking(ctx context.Context, input *bookingInputNormalized, requesterID string, managerID *string, status string, managerComment *string) (*Booking, error) {
	id := newUUID()
	approvedAt := any(nil)
	if status == "APPROVED" { approvedAt = time.Now().UTC() }
...
func nullableString(value *string) any {
	if value == nil || strings.TrimSpace(*value) == "" { return nil }
...
func (app *App) notifyRequester(ctx context.Context, booking *Booking, typ, title, detail string) error {
	...
	if detail != "" { message = booking.Title + "\n" + detail }
...
func (app *App) notifyAssignedBA(ctx context.Context, booking *Booking, typ, title, detail string) error {
	if booking.BAID == nil { return nil }
	var userID sql.NullString
	if err := app.DB.Pool.QueryRow(ctx, `select user_id from ba_profiles where id = $1`, *booking.BAID).Scan(&userID); err != nil || !userID.Valid { return nil }
	message := booking.Title
	if detail != "" { message = booking.Title + "\n" + detail }
