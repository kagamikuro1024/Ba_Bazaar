...
func (app *App) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if user.Status == "DISABLED" {
		writeJSON(w, http.StatusForbidden, map[string]string{"message": "Your account has been disabled. Please contact an IT administrator."})
		return
	}
...
func (app *App) handleRefresh(w http.ResponseWriter, r *http.Request) {
	...
	const q = `
		select rt.id, rt.user_id, rt.expires_at, rt.revoked_at,
		       u.id, u.full_name, u.email, u.role::text, u.password_hash, u.avatar_url, u.last_login_at, u.status::text
	...
	err := app.DB.Pool.QueryRow(r.Context(), q, hashRefreshToken(strings.TrimSpace(req.RefreshToken))).Scan(
		...
		&user.ID, &user.FullName, &user.Email, &user.Role, &pass, &avatar, &last, &status,
	)
