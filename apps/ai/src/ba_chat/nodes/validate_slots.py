...
async def ask_missing(state: ChatState) -> ChatState:
    ...
    field = missing[0]
    question = _FIELD_PROMPTS.get(field, f"I need the {field.replace('_', ' ')} for this booking.")
    actions = _FIELD_ACTIONS.get(field, [])

    msg = AIMessage(content=question)
    # Attach action buttons as additional_kwargs so the server can forward them
